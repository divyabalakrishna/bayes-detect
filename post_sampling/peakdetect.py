...
i = 10000
x,y,r,a,L = loadtxt("../files/0/"+"0_out_points_som.txt", unpack=True)
...
def _peakdetect_parabole_fitter(raw_peaks, x_axis, y_axis, points):
    ...
    for peak in raw_peaks:
        ...
        x2 = linspace(x_data[0], x_data[-1], points * 10)
        y2 = func(x2, *popt)
...
def peakdetect(y_axis, x_axis = None, lookahead = 300, delta=0):
    ...
    for index, (x, y) in enumerate(zip(x_axis[:-lookahead], 
        ...
        if y < mx-delta and mx != Inf:
            ...
            #    mx = ahead
            #    mxpos = x_axis[where(y_axis[index:index+lookahead]==mx)]
        ...
        if y > mn+delta and mn != -Inf:
            ...
            #else:  #slows shit down this does
            #    mn = ahead
            #    mnpos = x_axis[where(y_axis[index:index+lookahead]==mn)]
...
def peakdetect_fft(y_axis, x_axis, pad_len = 5):
    ...
    for peaks in [max_peaks, min_peaks]:
        ...
        index = 0
        for peak in peaks:
            index = where(x_axis_ifft[index:]==peak[0])[0][0] + index
...
def peakdetect_sine(y_axis, x_axis, points = 9, lock_frequency = False):
    ...
    # get global offset
    offset = mean([mean(max_raw, 0)[1], mean(min_raw, 0)[1]])
    ...
    if lock_frequency:
        func = lambda x, A, tau: A * sin(2 * pi * Hz * (x - tau) + pi / 2)
    else:
        func = lambda x, A, Hz, tau: A * sin(2 * pi * Hz * (x - tau) + 
                                                pi / 2)
    #func = lambda x, A, Hz, tau: A * cos(2 * pi * Hz * (x - tau))
...
def peakdetect_zero_crossing(y_axis, x_axis = None, window = 11):
    ...
    if abs(even_bins_y[0].max()) > abs(even_bins_y[0].min()):
        ...
        for bin_x, bin_y, peak in zip(even_bins_x, even_bins_y, hi_peaks):
            hi_peaks_x.append(bin_x[where(bin_y==peak)[0][0]])
        for bin_x, bin_y, peak in zip(odd_bins_x, odd_bins_y, lo_peaks):
            lo_peaks_x.append(bin_x[where(bin_y==peak)[0][0]])
    else:
...
def zero_crossings(y_axis, window = 11):
    ...
    # smooth the curve
    length = len(y_axis)
    x_axis = asarray(range(length), int)
    
    # discard tail of smoothed signal
    y_axis = _smooth(y_axis, window)[:length]
    zero_crossings = where(diff(sign(y_axis)))[0]
    ...
    #return indices + asarray(30 * random.randn(len(indices)), int)
...
def _test_graph():
    i = 10000
    x = linspace(0,3.7*pi,i)
    y = (0.3*sin(x) + sin(1.3 * x) + 0.9 * sin(4.2 * x) + 0.06 *
    random.randn(i))
    y *= -1
    x = range(i)
